 ...
 *
 * It contains the MapDouble, which is a map that may have any type as keys, and double as
 * values.
 *
...
/**
 * The MapDouble is a map that may have any type as keys, and {@code double} as values.
 * 
 ...
 * @version 1.0
 */
public interface MapDouble<K> extends Iterable<MapDouble.Entry<K>> {
     ...
     * one mapping to a value {@code v} such that {@code Double.compare(value, v) == 0}.
     * This operation will probably require time linear in the map size for most
     * implementations of the {@code MapDouble} interface.
     *
     ...
     * {@code OptionalDouble.empty()} if this map contains no mapping for the key.
     *
     * More formally, if this map contains a mapping from a key {@code k} to a value
     * {@code v} such that {@code Objects.equals(key, k)}, then this method returns
     * {@code OptionalDouble.of(v)}; otherwise it returns {@code OptionalDouble.empty()}.
     * There can be at most one such mapping.
     *
     * @param key The key whose associated value is to be returned.
     * 
     ...
     *         not permit {@code null} keys.
     */
    OptionalDouble safeGet(Object key);
     ...
     *         {@code OptionalDouble.empty()} if there was no mapping for {@code key}.
     *
     ...
     */
    OptionalDouble put(K key, double value);

    /**
     * If the specified key is not already associated with a value, associates it with the
     * given value and returns {@code OptionalDouble.empty()}, else returns the current
     * value.
     *
     * @param key The key with which the specified value is to be associated.
     * @param value The value to be associated with the specified key.
     ...
     *         {@code OptionalDouble.empty()} if there was no mapping for the key.
     *
     * @throws UnsupportedOperationException If the {@code put} operation is not supported
     *         by this map.
     * @throws ClassCastException If the key is of an inappropriate type for this map.
     ...
     *           method and document its concurrency properties.
     */
    default OptionalDouble putIfAbsent(K key, double value) {
     ...
     * The effect of this call is equivalent to that of calling {@link #put(Object, double)
     ...
     */
    void putAll(MapDouble<? extends K> map);
     ...
     *
     * <pre>
     * {@code map.merge(key, value, Double::sum)}
     * </pre>
     *
     * <p>
     ...
     *           method and document its concurrency properties.
     */
    default double merge(K key, double value, DoubleBinaryOperator remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        var oldValue = safeGet(key);
        double newValue = oldValue.isPresent()
                ? remappingFunction.applyAsDouble(oldValue.getAsDouble(), value)
                : value;
        put(key, newValue);
        return newValue;
    }
     ...
     * or {@code OptionalDouble.empty()} if the map contained no mapping for the key.
     *
     * <p>
     ...
     *         not permit {@code null} keys.
     */
    OptionalDouble remove(Object key);
    ...
    default boolean remove(Object key, double value) {
        var oldValue = safeGet(key);
        if (oldValue.isPresent() && (Double.compare(oldValue.getAsDouble(), value) == 0)) {
            remove(key);
            return true;
        }
     ...
     *           method and document its concurrency properties.
     */
    default OptionalDouble replace(K key, double value) {
    ...
    default boolean replace(K key, double oldValue, double newValue) {
        var currentValue = safeGet(key);
        if (!currentValue.isPresent() || (Double.compare(currentValue.getAsDouble(), oldValue) != 0)) {
            return false;
        }
        put(key, newValue);
        return true;
    ...
    /**
     * A map-double entry (key-value pair).
     * The {@code MapDouble.entrySet} method returns a collection-view of the map, whose
     * elements are of this class.
     * {@code MapDouble.Entry} objects are valid <i>only</i> for the duration of the
    ...
    interface Entry<K> {
         ...
         * 
         * <pre>
         * ((e.getKey() == null) ? 0 : e.getKey().hashCode()) ^ Double.hashCode(e.getValue())
         * </pre>
         * 
         ...
         *         &amp;&amp; (Double.compare(e1.getValue(), e2.getValue()) == 0);
         * </pre>
         * 
         * This ensures that the {@code equals} method works properly across different
         * implementations of the {@code MapDouble.Entry} interface.
         *
     ...
     *           method and document its concurrency properties.
     */
    default void forEach(ObjDoubleConsumer<? super K> action) {
